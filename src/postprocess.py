...
class PostProcessor:
        ...
        for i, path in enumerate(sample_paths):
            ...
            try:
                ...
                if operations.get('trim_silence'):
                    ...
                    audio_data = self._trim_silence(audio_data, samplerate, threshold_db)
                    print(f"  - Trimmed silence (threshold: {threshold_db:.1f}dB)")
    ...
    def _trim_silence(self, audio_data: np.ndarray, samplerate: int,
                      threshold_db: float = -60.0, margin_ms: float = 10.0) -> np.ndarray:
        """
        ...
        Args:
            ...
            samplerate: Sample rate in Hz
            threshold_db: Silence threshold in dB (default -60dB for very quiet samples)
    ...
    def _find_loop_points(self, audio_data: np.ndarray, samplerate: int,
        ...
        if len(sustain_region) < min_loop_length * samplerate:
            print("  - Sustain region too short for minimum loop length")
        ...
        if result is not None:
            loop_length, _ = result  # _ for unused quality score
            ...
            if loop_start < sustain_start:
                ...
                print("  - Loop clipped to fit sustain region")
